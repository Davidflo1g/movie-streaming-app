    ...
    <div class="w3-container w3-black">
        ...
        <?php
            ...
            if (isset($_POST['movie_id'])) {
                ...
                <form method="POST" class="w3-container w3-light-grey w3-margin-top">
                    <fieldset>
                        ...
                        <label>Title</label>
                        <input type="text" name="title" class="w3-input w3-border" value="<?php echo $movie['title']; ?>"
                            required>
                        ...
                        <input type="text" name="release_year" class="w3-input w3-border"
                            value="<?php echo $movie['release_year']; ?>" required>

                        <label>Duration (seconds)</label>
                        <input type="text" name="secs" class="w3-input w3-border" value="<?php echo $movie['secs']; ?>"
                            required>

                        <label>Director</label>
                        <input type="text" name="director" class="w3-input w3-border" value="<?php echo $movie['director']; ?>"
                            required>

                        <label>Description</label>
                        <textarea name="description" class="w3-input w3-border"
                            required><?php echo $movie['description']; ?></textarea>

                        <label>YouTube Trailer URL</label>
                        <input type="text" name="trailer_url" class="w3-input w3-border"
                            value="<?php echo $movie['trailer_source']; ?>" required>
                    </fieldset>
            ...
            if (isset($_POST['update'])) {
                ...
                $secs = mysqli_real_escape_string($conn, $_POST['secs']);
                $director = mysqli_real_escape_string($conn, $_POST['director']);
                $desc = mysqli_real_escape_string($conn, $_POST['description']);
                $trailer = mysqli_real_escape_string($conn, $_POST['trailer_url']);

                $update = "UPDATE movies 
                    SET title='$title', genre_id='$genre', release_year='$year', secs='$secs', 
                        director='$director', description='$desc', trailer_source='$trailer'
                    WHERE movie_id='$id'";
